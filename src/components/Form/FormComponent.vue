<template>
  <div v-if="getCurrentSelect !== ''" id="static-modal" data-modal-backdrop="static" tabindex="1" aria-hidden="false"
    ...
    <div class="p-20 rounded-lg bg-gray-100 w-1/2 relative">
      ...
      <button @click="setToStore('')" type="button"
      ...
      <form class="space max-w-full p-5">
        <label id="name" class="w-full text-left relative text-gray-900" for="name">Имя:</label><br>
        <input v-model="formFields.name"
          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          type="text" id="name" name="name"><br>
        <label id="phone" class="w-full text-left relative text-gray-900" for="phone">Телефон:</label><br>
        <input @input="addMask()" v-model="formFields.phone"
          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          maxlength="18" type="tel" id="phone" name="phone"><br>
        <label id="email" class="w-full text-left relative text-gray-900" for="email">Email:</label><br>
        <input v-model="formFields.email"
          class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          type="email" id="email" name="email"><br>
        <label id="city" class="w-full text-left relative text-gray-900" for="city">Город:</label><br>
        <FormSelect v-model="formFields.city" :allCities="getCities" :currenSelect="getCurrentSelect" />
        <button @click="validateForm"
          class="cursor-pointer hover:bg-gray-50 hover:border-blue-100 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">Отправить</button>
      </form>
    </div>
  </div>
</template>
...
const store = useStore,
  emit = defineEmits<{
    (event: 'send', data: typeof formFields): void
  }>()
...
const validateForm = (e: Event) => {
  ...
  for (let item in formFields.value) {
    ...
    if (value === null || value.length < minFieldLength[item]) {
      ...
      //@ts-ignore
      textLabel?.insertAdjacentHTML("beforeend", `<div class="tooltip-error w-full absolute top-5 text-red-500">${errors[item]}</div>`);
    }
  }
